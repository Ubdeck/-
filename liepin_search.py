...
class LiepinSearchPage:
    ...
    def apply_filters(self, filters: SearchFilters) -> dict:
        ...
        if filters.education:
            self.click_row_option(EDUCATION_TITLE, filters.education)
    ...
    def process_candidate_batch(self, filters: SearchFilters) -> dict:
        ...
        for index in range(limit):
            ...
            if decision.get("communicate_status") == "done":
                resume_targets.append(
                    {
                        "index": index + 1,
                        "name": result_item["name"],
                        "job_position": result_item["job_position"],
                        "job_cities": result_item["job_cities"],
                        "chat_job": filters.selected_chat_job or {},
                    }
                )
            elif decision.get("communicate_status") == "already_communicated":
                self.progress.emit(
                    "resume_request_skip",
                    f"第 {index + 1}/{limit} 个此前已沟通过，本轮不索要简历",
    ...
    def process_current_candidate(self, filters: SearchFilters) -> dict:
        ...
        decision = self.decide_candidate_match(profile, filters)
        profile_path = APP_DIR / "candidate_profile.json"
        ...
        while time.time() < deadline:
            last_result = self.page.run_js(
                ...
                const alreadyCandidates = candidates
                  ...
                  .filter(item => /觉得TA还不错|获取电话|意向沟通|超级聊聊/.test(ancestorText(item.target)));
                ...
                candidates.sort((a, b) => {
                  ...
                  if (aExact !== bExact) return aExact - bExact;
                  const aContext = ancestorText(a.target);
                  const bContext = ancestorText(b.target);
                  const aSidePanel = /觉得TA还不错|获取电话|意向沟通|超级聊聊/.test(aContext) ? 0 : 1;
                  const bSidePanel = /觉得TA还不错|获取电话|意向沟通|超级聊聊/.test(bContext) ? 0 : 1;
                  if (aSidePanel !== bSidePanel) return aSidePanel - bSidePanel;
